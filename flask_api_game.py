...
def render_court_response():
    attacker = request.json.get('attacker')
    judge_response = user_input(attacker, prompt_template_definition="""The question given to you is an argument kept by the attacker and the agent which is you, acts as a judge using the Constitution of India and the context given to you. Give reasonable objections, interventions, and logical outputs. The output is to be only 2 lines.\n\nContext:\n{context}?\nQuestion:\n{question}\nAnswer:""")
    defendent_response = user_input(attacker, prompt_template_definition="""You, the agent, are a defendant in the courtroom. You are an argument against your client. Get the information of the case from your context and make arguments against the attacker using the context and the Constitution of India. The output is to be of only 3 lines.\n\nContext:\n{context}?\nQuestion:\n{question}\nAnswer:""")
    return jsonify({"judge":judge_response,"deffender":defendent_response})
...
def render():
    ...
    judge_response = ""
    main_count = 10
    show_summary_before_render(number=2)
    while main_count != 0:
        render_court_response()
        main_count -= 1
        

    judge_response = user_input("Summary of all the previous chats and the judgment of u as a judge",prompt_template_definition="""Based on the previous chats consider them as arguments for your courtroom as you are the judge. Give the judgment based on the previous arguments, and give a three-line constitutional reference for the judgment from the Indian Constitution.\n\nContext:\n{context}?\nQuestion:\n{question}\nAnswer:""")

    return jsonify({
        "summary": summary,
        "judge": judge_response,
        "defendant": defendent_response
    })
@app.route('/summary',methods=['GET'])
def show_summary_before_render():
    number = random.randint(0,18)
    pdf_path = make_model_database(number)
